...
class ReportController extends Controller
{
    public function index()
    {
        ...
        $leads = Lead::where('seller_id', auth()->guard('seller')->user()->id)->where('type', 'regular')->count();
        ...
        $shipped = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('shipment_status', 'shipping')->count();
        $delivered = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('shipment_status', 'delivered')->count();
        $returned = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('shipment_status', 'returned')->count();
    ...
    public function filter_country($countryId)
    {
        ...
        $leads = Lead::whereIn('id', $lead_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('warehouse', $country->name)->count();
        ...
        $invoice_balance = Invoice::where('seller_id', auth()->guard('seller')->id())->sum('revenue');
        $balance = $invoice_balance + $revenue_confirmed;
        $shipped = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->whereHas('lead', function ($query) use ($country) {
            $query->where('warehouse', $country->name);
        })->where('shipment_status', 'shipping')->count();
        $delivered = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->whereHas('lead', function ($query) use ($country) {
            $query->where('warehouse', $country->name);
        })->where('shipment_status', 'delivered')->count();
        $returned = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->whereHas('lead', function ($query) use ($country) {
            $query->where('warehouse', $country->name);
        ...
        if ($request->has('date') && $request->date != '') {
            ...
            if (count($dates) === 2) {
                ...
                if ($id == 0) {
                    ...
                    $under_process = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('shipment_status', 'pending')
                        ->whereBetween('created_at', [$start_date, $end_date])
                        ->count();

                    $confirmed = Lead::whereIn('id', $lead_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('status', 'confirmed')
                        ->whereBetween('created_at', [$start_date, $end_date])
                        ->count();

                    $canceled = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('shipment_status', 'canceled')
                        ->whereBetween('created_at', [$start_date, $end_date])
                        ->count();
                    ...
                    $shipped = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('shipment_status', 'shipping')
                        ->whereBetween('created_at', [$start_date, $end_date])
                        ->count();

                    $delivered = Order::whereIn('lead_id', $orders_ids)->where('shipment_status', 'delivered')
                        ->whereBetween('created_at', [$start_date, $end_date])
                        ->count();

                    $returned = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->where('shipment_status', 'returned')
                        ->whereBetween('created_at', [$start_date, $end_date])
                        ->count();
                } else {
                    ...
                    $confirmed = Lead::whereIn('id', $lead_ids)->where('seller_id', auth()->guard('seller')->user()->id)->whereBetween('created_at', [$start_date, $end_date])
                        ->where('warehouse', $country->name)->where('status', 'confirmed')->count();

                    $canceled = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->whereBetween('created_at', [$start_date, $end_date])
                    ...
                    $invoice_balance = Invoice::where('seller_id', auth()->guard('seller')->id())->sum('revenue');
                    $balance = $invoice_balance + $revenue_confirmed;
                    $shipped = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->whereBetween('created_at', [$start_date, $end_date])
                        ->whereHas('lead', function ($query) use ($country) {
                            $query->where('warehouse', $country->name);
                        })->where('shipment_status', 'shipping')->count();
                    $delivered = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->whereBetween('created_at', [$start_date, $end_date])
                        ->whereHas('lead', function ($query) use ($country) {
                            $query->where('warehouse', $country->name);
                        })->where('shipment_status', 'delivered')->count();
                    $returned = Order::whereIn('lead_id', $orders_ids)->where('seller_id', auth()->guard('seller')->user()->id)->whereBetween('created_at', [$start_date, $end_date])
                        ->whereHas('lead', function ($query) use ($country) {
                            $query->where('warehouse', $country->name);
                        })->where('shipment_status', 'returned')->count();
