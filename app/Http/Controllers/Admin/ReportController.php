...
class ReportController extends Controller
{
    ...
    public function index()
    {
        if (auth()->user()->is_manager == 1) {
            ...
            $leads = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'regular')->count();
            $lead_ids = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'regular')->pluck('id');
            ...
            $revenue_confirmed  = Revenue::whereIn('seller_id', $seller_manager_ids)->sum('revenue');
            $invoice_balance = Invoice::whereIn('seller_id', $seller_manager_ids)->sum('revenue');
    ...
    public function filter(Request $request, $id)
    {
        if (auth()->user()->is_manager == 1) {
            ...
            $orders = Order::whereIn('seller_id', $seller_manager_ids)->count();
            $countries = Country::all();
            $country = 0;
            ...
            $lead_ids = Lead::WhereBetween('order_date', [$start_date, $end_date] ?? [])
                ->orWhereIn('seller_id', $sellerIds ?? [])
                ->Where('type', 'regular')
                ->pluck('id');

            $orders_ids = Order::whereIn('lead_id', $lead_ids)->orWhereIn('seller_id', $sellerIds ?? [])->pluck('id');
            ...
            if ($id == 0) {
                ...
                $under_process = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date, $sellerIds) {
                ...
                $canceled = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date, $sellerIds) {
                ...
                $delivered = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date, $sellerIds) {
                ...
                $returned = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date, $sellerIds) {
                ...
                $under_process = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date, $sellerIds, $leadsId) {
                    $query->where('shipment_status', 'pending')
                ...
                $canceled = Order::whereIn('lead_id', $orders_ids)->where('shipment_status', 'canceled')
                    ->WhereBetween('created_at', [$start_date, $end_date] ?? [])
                    ->orWhereIn('seller_id', $sellerIds ?? [])

                    ->count();

                $canceled = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date, $sellerIds, $leadsId) {
                    $query->where('shipment_status', 'canceled')
                        ->where(function ($query) use ($start_date, $end_date, $sellerIds, $leadsId) {
                            $query->WhereBetween('created_at', [$start_date, $end_date] ?? [])
                                ->orWhereIn('seller_id', $sellerIds ?? [])
                ...
                $balance = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date,  $sellerIds, $leadsId) {
                ...
                $delivered = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date, $sellerIds, $leadsId) {
                    $query->where('shipment_status', 'delivered')
                        ->where(function ($query) use ($start_date, $end_date, $sellerIds, $leadsId) {
                            $query->WhereBetween('created_at', [$start_date, $end_date] ?? [])
                                ->orWhereIn('seller_id', $sellerIds ?? [])
                ...
                $returned = Order::whereIn('lead_id', $orders_ids)->where(function ($query) use ($start_date, $end_date, $sellerIds, $leadsId) {
                    $query->where('shipment_status', 'returned')
                        ->where(function ($query) use ($start_date, $end_date, $sellerIds, $leadsId) {
                            $query->WhereBetween('created_at', [$start_date, $end_date] ?? [])
                                ->orWhereIn('seller_id', $sellerIds ?? [])
    ...
    public function affiliate_filter()
    {
        if (auth()->user()->is_manager == 1) {
            ...
            $leads = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'commission')->count();
            $lead_ids = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'commission')->pluck('id');
    ...
    public function marketplace()
    {
        if (auth()->user()->is_manager == 1) {
            ...
            $affiliateSkus = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'commission')->distinct()->pluck('item_sku');
            ...
            $sharedSkus = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'regular')->distinct()->pluck('item_sku');
    ...
    public function markplace_filter_date(Request $request)
    {
        if (auth()->user()->is_manager == 1) {
            ...
            $affiliateSkus = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'commission')->WhereBetween('created_at', [$start_date, $end_date] ?? [])->distinct()->pluck('item_sku');
            ...
            $sharedSkus = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'regular')->whereBetween('created_at', [$start_date, $end_date] ?? [])->distinct()->pluck('item_sku');
    ...
    public function affiliate_filter_date(Request $request)
    {
        if (auth()->user()->is_manager == 1) {
            ...
            $leads = Lead::whereIn('seller_id', $seller_manager_ids)->where('type', 'commission')->whereBetween('order_date', [$start_date, $end_date] ?? [])->count();
