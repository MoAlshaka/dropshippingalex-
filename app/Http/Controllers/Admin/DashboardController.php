...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if (auth()->user()->is_manager == 0) {
            ...
            $leads = Lead::whereIn('seller_id', $seller_manager_ids)->count();
            // dd($leads);
            $approvedLeadsCount = Lead::whereIn('seller_id', $seller_manager_ids)->where('status', 'confirmed')->count();
            $deliveredLeadsCount = Order::whereIn('seller_id', $seller_manager_ids)->where('shipment_status', 'delivered')->count();
            $pendingLeadsCount = Lead::whereIn('seller_id', $seller_manager_ids)->where('status', 'pending')->count();
            $revenue = 0;
            $revenues = Revenue::orderBy('revenue', 'desc')
                ->limit(10)
                ->get();
            ...
            $minMaxDates = Lead::whereIn('seller_id', $seller_manager_ids)->selectRaw('MIN(order_date) as min_date, MAX(order_date) as max_date')->first();
            ...
            // Execute the query to get counts for existing dates
            $existingCounts = Lead::whereIn('seller_id', $seller_manager_ids)->selectRaw('order_date, COUNT(*) as count')
                ->groupBy('order_date')
                ->pluck('count', 'order_date')
                ->toArray();
            // Fill in missing counts with zero
            ...
            // orders
            // Charts for Orders
            $minMaxOrderDates = Order::whereIn('seller_id', $seller_manager_ids)->selectRaw('MIN(created_at) as min_date, MAX(created_at) as max_date')->first();
            $startOrderDate = Carbon::parse($minMaxOrderDates->min_date)->format('Y-m-d');
            $endOrderDate = now()->toDateString();
            ...
            // Execute the query to get counts for existing dates
            $existingOrderCounts = Order::whereIn('seller_id', $seller_manager_ids)->selectRaw('DATE(created_at) as order_date, COUNT(*) as count')
                ->where('shipment_status', 'delivered')
                ->groupBy('order_date')
                ->pluck('count', 'order_date')
                ->toArray();
        ...
        return view('admin.dashboard', compact('leads', 'pendingLeadsCount', 'approvedLeadsCount', 'deliveredLeadsCount', 'revenue', 'sellers', 'leads_count', 'orders_count', 'all_sellers', 'admins', 'allSellers'));
    }
