        ...
        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-8 gap-4 p-4">
            <div class="product-card col-span-8 lg:col-span-3 py-2 px-4 rounded-md drop-shadow bg-white">
                <div class="product-img flex mb-4">
                    <img src="{{ asset('assets/products/sharedproduct/images/' . $product->image) }}" alt=""
                        class="w-full h-full object-cover" />
                </div>
                <div>
                    @php
                        $imported = false;
                        foreach ($product->sharedSellers as $seller) {
                    ...
                    @if ($imported === true)
                        <form action="{{ route('seller.sharedproduct.exclude', $product->id) }}" method="POST">
                            @csrf
                    ...
                    @else
                        <form action="{{ route('seller.sharedproduct.import', $product->id) }}" method="POST">
                            @csrf
            ...
            <div class="product-disc col-span-8 lg:col-span-5 bg-white rounded-md drop-shadow">
                ...
                <div class="px-2">
                    ...
                    <hr />
                    <div class="py-6 px-2">
                        <h3 class="text-md font-bold mb-4"> {{ __('site.Specification') }}:</h3>
                ...
                <table class="table">
                    <thead>
                        <tr class="text-nowrap">
                            ...
                            <th> {{ __('site.UnitCost') }}</th>
                            <th> {{ __('site.RecommendedPrice') }}</th>
